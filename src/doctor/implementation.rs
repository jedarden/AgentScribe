...
fn check_last_scrape_recent(data_dir: &Path, debounce_seconds: u64) -> CheckResult {
    ...
    let last_scrape: Option<DateTime<Utc>> =
    ...
    if elapsed.num_seconds() > stale_threshold_secs {
        CheckResult {
            name: "last_scrape_recent".to_string(),
            passed: false,
            message: format!("Last scrape was {} ago (stale)", format_duration(elapsed)),
            context: Some(format!(
    ...
    match parse_scrape_state(&state_file) {
        Ok(state) => {
            ...
            CheckResult {
                ...
                passed: true,
                message: format!(
                    "State file parses cleanly ({} sources tracked)",
                    source_count
                ),
                context: None,
                severity: "info".to_string(),
            ...
            let has_quarantine = quarantine_dir
                ...
                .map(|entries| {
                    entries.filter_map(|e| e.ok()).any(|e| {
                        e.file_name()
                            .to_string_lossy()
                            .starts_with("scrape-state.json.corrupt-")
                    })
                })
                .unwrap_or(false);
...
fn parse_scrape_state(path: &Path) -> Result<ScrapeState> {
    ...
    serde_json::from_reader(reader)
        .map_err(|e| AgentScribeError::State(format!("Parse error: {}", e)))
}
