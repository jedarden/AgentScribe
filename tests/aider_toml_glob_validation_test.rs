...
fn test_aider_toml_deserializes_without_error() {
    ...
    assert_eq!(
        ...
        agentscribe::plugin::LogFormat::Markdown
    );
}
...
fn test_aider_paths_contains_recursive_glob() {
    ...
    assert_eq!(
        plugin.source.paths[0], "~/**/.aider.chat.history.md",
...
fn test_glob_expansion_discovers_nested_files_and_excludes_correctly() {
    ...
    let pycache_dir = base.join("projects").join("python-app").join("__pycache__");
    ...
    std::fs::write(
        &top_file,
        "# aider chat started at 2024-02-01\n#### user\nassistant",
    )
    .unwrap();
    ...
    std::fs::write(
        &nm_file,
        "# aider chat started at 2024-03-01\n#### user\nassistant",
    )
    .unwrap();
    ...
    let discovered: Vec<_> = glob_result
        .filter_map(|e| e.ok())
        .filter(|p| p.is_file())
        .collect();
    ...
    assert_eq!(
        discovered.len(),
        6,
        "Should discover 6 files before exclusion"
    );
